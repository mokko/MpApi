...
USAGE:
    ...
    #dealing with items
    mi = ModuleItem(hasAttachments="false", id=254808)
    ml.item(item=i) # adds item
    i = ml.item(id=123) # gets item by id  
    rg = RepeatableGroup()
    ...
    ####
    What if all elements are part of the same python class?
    ####

    ml = ModuleList(name="Object", totalSize=1) # new Object item from scratch
    #ml is ModuleList object; following objects are lxml.etree objects
    ...
    ml.iter(group=rg) # can iter be generalized
...
class ModuleList (Helper):
    ...
    def attribute(self, *, parent=None, name, action=None, value=None):
        ...
        if parent is None:
            parent = self.etree
        itemL = parent.xpath("/m:application/m:modules/m:module/m:moduleItem", 
            namespaces=NSMAP)
        for itemN in itemL:
            if action == "remove":
    ...
    def dataField (self, *, parent, name, dataType=None, value=None):
        ...
        if value is None: pass
            ...
            dataFieldN = etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}dataField", dataType=dataType, name=name)
    ...
    def moduleItem(self, *, hasAttachments, id):
        ...
        """
        mi = etree.Element("{http://www.zetcom.com/ria/ws/module}moduleItem", 
            hasAttachments=hasAttachments, id=id)
        
        moduleItemN = self.etree.xpath("/m:application/m:modules/m:module[last()]", 
            namespaces=NSMAP)[0]
        moduleItemN.append(mi)
        return mi

    def moduleItemIter(self):
        itemsN = self.etree.xpath ("/m:application/m:modules/m:module/m:moduleItem", 
            namespaces=NSMAP)
        for itemN in itemsN:
            yield itemN
    ...
    def moduleReference (self, *, parent, name, targetModule, multiplicity, size):  
        ...
        """
        return etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}moduleReference", 
            name=name, multiplicity=multiplicity, taretModule=targetModule)
    ...
    def repeatableGroup (self, *, parent, name, size):
        ...
        """
        return etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}repeatableGroup", 
            name=name, size=size)

    def repeatableGroupItem (self, *, parent, id):
        return etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}repeatableGroupItem", 
            id=id)
    ...
    def systemField (self, *, parent, dataType, name, value=None): 
        ...
        if value is None: pass
            ...
            systemFieldN = etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}systemField", dataType=dataType, name=name)
    ...
    def vocabularyReference(self, *, parent, name, id, instanceName): 
        ...
        """
        return etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}vocabularyReference", 
            id=id, name=name, instanceName=instanceName)        
    ...
    def vocabularyReferenceItem (self, *, parent, name, id): 
        ...
        """
        return etree.SubElement(parent, "{http://www.zetcom.com/ria/ws/module}vocabularyReferenceItem", 
            id=id, name=name)
   ...
   #quick and dirty            
    def _rmUuidsInReferenceItems(self, *, parent=None):
        if parent is None:
            parent = self.etree
        itemL = parent.xpath("//m:moduleReference/m:moduleReferenceItem", 
            namespaces=NSMAP)
        for eachN in itemL:
...
if __name__ == "__main__":
    def load_file():
        ...
        for mi in ml.moduleItemIter(): 
            ...
            ml._dropFields(parent=mi, type="virtualField") #if no parent, assume self.etree
            ml._dropFields(parent=mi, type="systemField") #if no parent, assume self.etree
    ...
    def from_scratch():
        ...
        vr = ml.vocabularyReference(parent=mi, name="ObjCategoryVoc", id="30349", 
