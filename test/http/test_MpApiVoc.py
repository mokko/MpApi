...
def test_vInfo():
    ...
    write_file("../sdata/vInfo.xml", api, r)


def test_vGetNodes():
    api = MpApi(baseURL=baseURL, user=user, pw=pw)
    r = api.vGetNodes(instanceName="GenLocationVgr")
    write_file("../sdata/vGetNodes.xml", api, r)
...
def test_vGetLabels():
    ...
    assert r.status_code == 200
    write_file("../sdata/vGetLabels.xml", api, r)
...
def test_vGetNodeClasses():
    ...
    assert r.status_code == 200
    write_file("../sdata/vGetNodeClasses.xml", api, r)
...
def test_vNodeByIdentifier():
    ...
    assert r.status_code == 200
    write_file("../sdata/vGetNodeId4220572.xml", api, r)
